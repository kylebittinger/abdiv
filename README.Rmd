...
We'll read the tree in Newick format using the `ape` package. To render it just
like the figure, we arrange the tips of the tree in order from "a" to "e".
...
paper by Lozupone and Knight (PMID 16332807). We've stored the tree from
Figure 1 in our package files under `extdata/unifrac_paper.tre`, so we'll
load it from there.
...
The circle and square communities are not very different, phylogenetically. In
fact, if we measure the branch length leading uniquely to either a circle or a
square, we sum up the branch lengths on the far right-hand side of the plot.
This is about half the total branch length for the tree.
...
This value is the unweighted UniFrac distance. We can increase the UniFrac
distance if we communities so that all the circles are in the upper part of the
tree and all the squares are in the lower part, as in Figure 1B.
...
One practical matter issue with phylogenetic diversity is to make sure that
the order of the species in your vectors matches the order of the tree. To
accomplish this, you can obtain the order of species names from the tree
object:
